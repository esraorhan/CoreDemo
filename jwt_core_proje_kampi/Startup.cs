...
namespace jwt_core_proje_kampi
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            //jwt için gereklli konfigrasyon iþlemleri
            services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme).AddJwtBearer(x =>
            {
                x.RequireHttpsMetadata = false;  //htttp dahil edilmediðinden gerek mediðini belirtmemiz gereken yer 
                x.TokenValidationParameters = new TokenValidationParameters
                {
                    ValidIssuer = "http://localhost", //çalýsacaðýn alan 
                    ValidAudience = "http://localhost", //kullanýlacak taraf
                    IssuerSigningKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes("aspnetcreprojekampi")), //Token anahtar kodu olacak .
                    ValidateIssuerSigningKey = true, //token çözümlenemesi içn 
                    ValidateLifetime=true,//zaman farký olþmasýn
                    ClockSkew=TimeSpan.Zero
                };
            }); //jwt konfigirasyon yapýlan yer
        }
